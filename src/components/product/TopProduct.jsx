...
import React, { useEffect, useRef } from "react"; // useRef ko import karein
import { gsap } from "gsap";
import { ScrollToPlugin } from "gsap/ScrollToPlugin";
import { ScrollTrigger } from "gsap/ScrollTrigger"; // <<< CHANGE 1: ScrollTrigger ko import karein
import { motion } from "framer-motion";

// <<< CHANGE 2: Dono plugins ko register karein
gsap.registerPlugin(ScrollToPlugin, ScrollTrigger);

const TopProduct = () => {
  // <<< CHANGE 3: Paragraph element ke liye ek ref banayein
  ...
  const products = [
    {
      ...
      price: "$49",
      image: "/product-1.png",
      bgColor: "linear-gradient(to bottom, #dbeafe, #eff6ff)",
    },
    {
      id: "game-royal",
      name: "Diesel Perfect Sports",
      ...
      price: "$56",
      image: "/product-2.png",
      bgColor: "linear-gradient(to bottom, #fee2e2, #fef2f2)",
    },
    {
      id: "pine-green",
      name: "New Zealand",
      ...
      price: "$48",
      image: "/product-6.png",
      bgColor: "linear-gradient(to bottom, #fef9c3, #fffbeb)",
    },
    {
      id: "court-purple",
      name: "RYSE BCAA/EAA",
      ...
      price: "$40",
      image: "/product-4.png",
      bgColor: "linear-gradient(to bottom, #dcfce7, #f0fdf4)",
    },
  ];

  // <<< CHANGE 4: Blur effect ke liye alag se useEffect banayein
  useEffect(() => {
    // Check karein ki element maujood hai
    if (!blurTextRef.current) return;

    // Initial state set karein (shuru me kaisa dikhega)
    gsap.set(blurTextRef.current, {
      opacity: 0,
      filter: "blur(5px)",
      y: 20, // Thoda sa neeche se upar aane ka effect
    });

    // Scroll par animation banayein
    const tl = gsap.to(blurTextRef.current, {
      opacity: 1,
      filter: "blur(0px)",
      y: 0,
      duration: 0.8,
      ease: "power2.out",
      scrollTrigger: {
        trigger: blurTextRef.current, // Trigger element
        start: "top 90%", // Jab element ka top viewport ke 90% par aaye
        end: "bottom 20%",
        toggleActions: "play none none none", // Sirf ek baar play hoga
      },
    });
  ...
  }, []); // Empty array ka matlab ye effect sirf ek baar chalega

  // Card animations ke liye pehle wala useEffect
  useEffect(() => {
    const mainScrollContainer = document.querySelector(
      ".main-scroll-container"
    );
  ...
  return (
    <div className="text-gray-800 min-h-screen w-full flex flex-col justify-center items-center bg-white">
      <header className="text-center py-12 px-4">
        ...
        {/* <<< CHANGE 5: Paragraph ko ref de aur class `blurText` hata sakte hain */}
        <p ref={blurTextRef} className="text-lg text-gray-600 mt-2">
          Our best-selling products with interactive animations.
        </p>
      </header>
